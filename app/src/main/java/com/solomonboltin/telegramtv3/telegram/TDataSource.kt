...
import com.solomonboltin.telegramtv3.telegram.TDataSource.UriFactory.PARAM_FILE_ID
...
class TDataSource(private val filesVM: FilesVM) : BaseDataSource(true) {
    ...
    lateinit var uriV: Uri
    private var bytesRead: Long = 0
    var transferListener: TransferListener? = null
    private val lock = Object()
    private val fileLock = Any()
    private val localFile = TdApi.File(0, 0, 0, LocalFile(), RemoteFile())
     ...
     * [DataSpec.length].
     */
    fun open1(dataSpec: DataSpec): Long {
        Log.i("TdSource open", "open started returning ${dataSpec.toString()}")

        uriV = dataSpec.uri
        fileId = parseInt(uri!!.getQueryParameter(UriFactory.PARAM_FILE_ID) ?: "-1")
        filesVM.seekFileOffset(fileId!!)
        file = filesVM.getFile(fileId!!)
        val fileSize = file!!.size.toLong()
        transferStarted(dataSpec)
        Log.i("TdSource", "open finished returning $fileSize")
        return fileSize
    }
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        val function: TdApi.Function = GetFile(fileId!!)
        val openLatch = CountDownLatch(1)
        val isOpening = AtomicBoolean(true)
        val response = AtomicReference<TdApi.Object>()
//        filesVM.clientVM.client.send(function) { `object` ->
//            synchronized(isOpening) {
//                if (isOpening.getAndSet(false)) {
//                    if (`object`.getConstructor() === File.CONSTRUCTOR) {
//                        filesVM.filesHandler(`object` as File)
//                    }
//                    response.set(`object`)
//                }
//            }
...
//        }
//        val result = response.get()
//            ?: throw Exception("getFile failed")
//        if (result is Error) throw Exception(
//            "getFile failed:" + result.toString()
//        )
        synchronized(fileLock) { file = filesVM.getFile(fileId!!) }
        transferStarted(dataSpec)
        Log.i("TdSource", "open returning ${file!!.size.toLong()}")

        return if (file!!.size != 0) file!!.size.toLong() else C.LENGTH_UNSET.toLong()
    }


    private fun getAvailableSize1(file: TdApi.File, offset: Long, length: Int): Long {
        Log.i("TdSource", "file")
        if (file.local.isDownloadingCompleted) {
            Log.i("TdSource", "available: all")
            return file.local.downloadedSize - offset

        } else if (offset >= file.local.downloadOffset && offset < file.local.downloadOffset + file.local.downloadedPrefixSize) {
            val available = file.local.downloadedPrefixSize - (offset - file.local.downloadOffset)
            Log.i("TdSource", "available: $available")
            return available

        }
        Log.i("TdSource", "available: 0")
        return 0
    }

    private fun getAvailableSize(file: TdApi.File, offset: Long, length: Int): Int {
        Log.i("TdSource", "getAvilable, $offset, $length $file")
        val available: Long
        available =
            if (file.local.isDownloadingCompleted) file.local.downloadedSize - offset else if (offset >= file.local.downloadOffset && offset < file.local.downloadOffset + file.local.downloadedPrefixSize) file.local.downloadedPrefixSize - (offset - file.local.downloadOffset) else return 0
        val res = 0.coerceAtLeast(Math.min(length.toLong(), available).toInt())

        Log.i("TdSource", "res: $res")
        return res
     ...
     */
    @OptIn(InternalCoroutinesApi::class)
    fun read1(buffer: ByteArray, bufferOffset: Int, buffeLength: Int): Int {
        // seek file offset
        // read file from offset to offset + length
        // add new bytes to buffer
        // return number of bytes read
        Log.i("TdSource", "read requesting $bufferOffset")
        synchronized(lock) {
            ...
            while (true) {
                ...
                Log.i("TdSource", "read entering loop")
                file = filesVM.getFile(fileId!!)
                Log.i("TdSource", "read current file: $file")
                Log.i("TdSource", "read available: $available")

                val offset = bytesRead
                available = getAvailableSize(file!!, offset, buffeLength).toLong()

                if (available != 0L && file != null) {
                    Log.i("TdSource", "read reading ")
                    if (offset > 0) {
                        openFile!!.seek(offset)
                    }
                    Log.i(
                        "TdSource",
                        "random access file opened file length: ${openFile!!.length()}"
                    )
                    Log.i(
                        "TdSource",
                        "read reading from offset: $bufferOffset to ${bufferOffset + available}"
                    )

                    val readCount = openFile!!.read(buffer, bufferOffset, available.toInt())

                    Log.i("TdSource", "reading done ")
                    bytesRead += readCount.toLong()

                    Log.i("TdSource", "read read $readCount bytes")
                    bytesTransferred(readCount)
                    return readCount
                } else {
                    Log.i("TdSource", "read waiting for file")
                    Thread.sleep(1000)
                }
            }
        }

        throw java.lang.Exception("This shouldest happen")
